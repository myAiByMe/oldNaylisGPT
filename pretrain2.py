...
CONFIG = {
    ...
    'adam_beta2'            : 0.95,
    'adam_eps'              : 1e-8,
    # Data — fichiers locaux produits par tokens.py
    # Pretrain d'abord, SFT ensuite (dans cet ordre exact)
    'pretrain_bin'          : './pretrain_tokens.bin',
    'sft_bin'               : './sft_tokens.bin',
    'val_tokens'            : 10_000_000,       # 10M tokens validation
    'warmup_ratio'          : 0.03,
    'decay_ratio'           : 0.15,
    'min_lr_ratio'          : 0.1,
    # Validation
...
print('=' * 70)
print('  Naylis v1 — Pretrain (IPT : pretrain → SFT)')
print('=' * 70)
if DEVICE == 'cuda':
...
# ── Liste des fichiers de données (pretrain puis SFT) ────────────────────────
#  Un fichier = un chunk entier. Pas de découpage.
#  Ordre garanti : pretrain en premier, SFT ensuite.
ALL_CHUNKS = []
for _phase, _path in [('pretrain', CONFIG['pretrain_bin']),
                      ('sft',      CONFIG['sft_bin'])]:
    if not os.path.exists(_path):
        print(f'  ⚠ Fichier introuvable : {_path}')
        continue
    _tok = os.path.getsize(_path) // 2   # uint16 → 2 octets par token
    _cid = len(ALL_CHUNKS)
    ALL_CHUNKS.append({'id': _cid, 'phase': _phase, 'file': _path, 'tokens': _tok})
    print(f'  [{_phase:>8}] chunk_{_cid} : {_path}  →  {_tok/1e9:.2f}B tokens')

if not ALL_CHUNKS:
    print(f'\nERREUR : aucun fichier de données trouvé.')
    print(f'  pretrain : {CONFIG["pretrain_bin"]}')
    print(f'  sft      : {CONFIG["sft_bin"]}')
    print('  → Lancer tokens.py d\'abord')
    sys.exit(1)

print(f'\n  {len(ALL_CHUNKS)} fichier(s) au total (pretrain → SFT) :')
for c in ALL_CHUNKS:
    print(f'    [{c["phase"]:>8}] chunk_{c["id"]} : {c["tokens"]/1e9:.2f}B tokens')
...
class BinChunk:
    """
    Charge un fichier .bin uint16 complet produit par tokens.py.
    RAM directe, fallback mmap si MemoryError.
    """
    def __init__(self, chunk_info: dict, seq_len: int, val_tokens: int):
        cid   = chunk_info['id']
        phase = chunk_info['phase']
        print(f'  Chargement chunk_{cid} [{phase}]  '
              f'{chunk_info["tokens"]/1e9:.2f}B tokens...')
        t0 = time.time()

        try:
            arr  = np.fromfile(chunk_info['file'], dtype=np.uint16)
            mode = 'RAM'
        except MemoryError:
            print('  MemoryError → fallback mmap')
            arr  = np.memmap(chunk_info['file'], dtype=np.uint16, mode='r')
            mode = 'mmap'
        ...
        # Shuffle reproductible par chunk
        seq_len_1 = seq_len + 1
        ...
        print(f'  chunk_{cid} [{phase}|{mode}] : {total/1e6:.0f}M tokens  '
...
def train_one_chunk(
    ...
    print(f'\n{"="*70}')
    print(f'  chunk_{chunk_info["id"]:03d}  [{chunk_info["phase"].upper()}]  '
          f'({chunk_info["tokens"]/1e9:.2f}B tokens)  '
          f'[{chunk_idx+1}/{len(ALL_CHUNKS)}]')
    print(f'{"="*70}')

    chunk = BinChunk(chunk_info, CONFIG['max_seq_len'], CONFIG['val_tokens'])
    ...
    pbar = tqdm(
        train_loader, desc=f'  [{chunk_info["phase"]}] chunk_{chunk_info["id"]:03d}',
    ...
    total_time += elapsed
    avg_loss    = chunk_loss / max(valid_batches, 1)
    print(f'\n  chunk_{chunk_info["id"]:03d} [{chunk_info["phase"]}] terminé | '
...
def main():
    ...
    global_step       = 0
    current_chunk_idx = 0       # index dans ALL_CHUNKS — reprend ici en cas de crash
    total_time        = 0.0
    chunk_start_step  = 0
    ...
    print(f'  Ordre : {len([c for c in ALL_CHUNKS if c["phase"]=="pretrain"])} chunks pretrain  '
          f'→  {len([c for c in ALL_CHUNKS if c["phase"]=="sft"])} chunks SFT')
    print('=' * 70)

    # ── Boucle principale : un chunk à la fois, dans l'ordre, sans répétition ──
    for chunk_idx in range(current_chunk_idx, len(ALL_CHUNKS)):
        chunk_info = ALL_CHUNKS[chunk_idx]
        is_resume  = (chunk_idx == current_chunk_idx and cp is not None)
        if not is_resume:
            chunk_start_step = global_step

        # Affiche la transition pretrain → SFT
        if chunk_idx > 0 and chunk_info['phase'] != ALL_CHUNKS[chunk_idx - 1]['phase']:
            print(f'\n{"*"*70}')
            print(f'  ★  TRANSITION : PRETRAIN → SFT  ★')
            print(f'{"*"*70}')
    ...
    ckpt_mgr.save(model, optimizers, scheduler, {
        'global_step'        : global_step,
        'current_chunk_idx'  : len(ALL_CHUNKS),   # marque comme terminé
        'total_training_time': total_time,
        'chunk_start_step'   : global_step,
    })
